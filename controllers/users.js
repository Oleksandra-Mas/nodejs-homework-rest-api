...
const { getUserByEmail, registerUser, getUserByEmailAndPassword, updateToken, changeAvatar, findByVerificationToken, updateVerificationToken } = require('../service/user');
...
const register = async (req, res, next) => {
    try {
        ...
        const created = await registerUser({ ...body, avatarURL, verificationToken });
        
        const email = { to: created.email, subject: 'Verification', html: `<p>/users/verify/${verificationToken}<p/>` };
...
const resendVerificationToken = async (req, res, next) => {
    try {
        ...
        const email = { to: mail, subject: 'Verification', html: `<p>/users/verify/${req.verificationToken}<p/>` };
        await sendEmail(email);
